...
void ConfigPorts(void) {
    ...
    TRISAbits.TRISA0 = 1; // Eingang
    ANSELAbits.ANSELA0 = 1; // RA0 ist analog, ANA0!!!
    TRISDbits.TRISD0 = 0; // RD6 ist TX2, also Ausgang!
}

void ConfigADC(void) {
    ADCON0bits.FM = 0; // linksbuendig
    ADCON0bits.CS = 1; // ADCRC-Takt, Konversion dauert so ca. 50 us
    ADPCH = 0x00; // Kanal: RA0 
}
...
void InitTimer0(void) {
    ...
    T0CON1bits.CKPS = 0b0101; // Prescaler, 1:1, Bits CKPS3 bis CKPS0
...
void convertAnythingThatIsAnalogToTheSuperiorDigitalForm() {
    ...
    while (ADCON0bits.GO); // Warten, dass ADC fertig
    resultHigh = ADRESH; // Bits 11 bis 4 vom Ergebnis
    resultLow = ADRESL; // Bits 3 bis 0 vom Ergebnis
    PIR1bits.ADIF = 0; // Wird gesetzt, wenn ADC fertig
    ...
    {
        U2TXB = resultHigh; // Zeichen in das Senderegister schreiben
    }
}
...
/*
 * 256ms / 0.2us = 1280000 Schritte <- Schritte ohne Prescaler
 * 1280000 / 65536 = 19.53125
 * 
 * 32 / 0.2us = 6.4us <- Mit 1:32 Prescaler
 * 
 * 256ms/6.4us = 40000 Schritte
 * 
 * 40000 = 0x63C0
 */