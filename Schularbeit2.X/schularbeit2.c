...
void InitTimer0(void)
{
    ...
    T0CON0bits.OUTPS = 0b0011;      // Postscaler 1:1, Bits OUTPS3 bis OUTPS0
    ...
    T0CON1bits.CKPS = 0b0000;       // Prescaler, 1:1, Bits CKPS3 bis CKPS0
...
void uebung01() {
    ...
    while(1)
    {
        ...
        {
            TMR0H = 0xCF; //TODO richtigen Wert herausfinden
            TMR0L = 0x2C;
            //RC0PPS0_bit = ~RC0PPS0_bit; 
            if(LATC = 0)
                LATC = 1;
            else
                LATC = 0;
...
void uebung02() {
    ...
    while(1)
    {
        ...
        if(PIR3bits.TMR0IF)         // Hier Polling = abfragen
        {
            TMR0H = 0x3C; //TODO richtigen Wert herausfinden
            TMR0L = 0xB0;
            LATC = ~LATC;
            PIR3bits.TMR0IF = 0;
        }  
...
void uebung03_sendChars() {
	if(PIR6bits.U2TXIF)	    // Ist das Senderegister frei?
        ...
        if(wasFirstCharSent == 0)
             U2TXB = 'R';	   // Zeichen in das Senderegister schreiben
        else 
            U2TXB = 'S';
        }
      __delay_ms(50);
